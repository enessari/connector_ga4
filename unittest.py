...
class TestGA4Connector(unittest.TestCase):
    ...
    def test_run_query_for_property_mock(self, mock_client):
        mock_instance = MagicMock()
        mock_client.return_value = mock_instance
        mock_instance.run_report.return_value.rows = []

        result = run_query_for_property(
    ...
    def test_save_query_results(self, mock_to_csv, mock_makedirs):
        dummy_data = [{"a": 1, "b": 2}]
        save_query_results(dummy_data, "analytics", "test_query")
    ...
    def test_discover_all_properties_handles_exception(self, mock_admin):
        ...
        result = discover_all_properties({"client_email": "test@test.com"})
        self.assertEqual(result, [])
